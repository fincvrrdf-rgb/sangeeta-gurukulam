/**
 * Register page — beautiful split-panel design matching brand aesthetic.
 */
...
export default function RegisterPage() {
  ...
  const assignStudentRole = async (uid: string, userEmail: string, name: string, idToken: string) => {
    const res = await fetch('/api/auth/set-role', {
      ...
      body: JSON.stringify({ targetUserId: uid, role: 'student', displayName: name, email: userEmail }),
    });
    if (!res.ok) {
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (message.includes('email-already-in-use')) setError('This email is already registered. Please sign in instead.');
      ...
      >
        ...
        <div className="relative z-10 text-center px-12">
          ...
          <div className="mt-10 grid grid-cols-3 gap-6 text-center">
            {[
              { icon: '🎵', label: '13 Geethams' },
              { icon: '🎙️', label: 'Live Classes' },
              { icon: '📖', label: 'AI Lyrics' },
            ].map(({ icon, label }) => (
      ...
      >
        <div className="w-full max-w-md">
          ...
          <div className="text-center mb-8">
            <h2 className="text-3xl font-bold text-amber-900">Join Us</h2>
            <p className="mt-2 text-amber-700 text-sm">Begin your Carnatic music journey today</p>
          </div>
          ...
          <div className="relative mb-6">
            ...
            <div className="relative flex justify-center">
              <span className="bg-amber-50 px-4 text-xs text-amber-600 font-medium">or register with email</span>
            </div>
          </div>

          <form onSubmit={handleEmailRegister} className="space-y-4">
            <div>
              <label htmlFor="name" className="block text-sm font-semibold text-amber-900 mb-1.5">
                Full Name
              </label>
              <input
            ...
            <div>
              <label htmlFor="password" className="block text-sm font-semibold text-amber-900 mb-1.5">
                Password
              </label>
              <input
            ...
            <div className="flex items-start gap-3 p-3 bg-amber-50 rounded-xl border border-amber-200">
              ...
              />
              <label htmlFor="terms" className="text-xs text-amber-800 cursor-pointer leading-relaxed">
                I accept the{' '}
                <Link href="/terms" className="font-semibold underline hover:text-amber-600">Terms of Service</Link>
                {' '}and{' '}
                <Link href="/privacy" className="font-semibold underline hover:text-amber-600">Privacy Policy</Link>.
                Class recordings will be stored securely for practice review.
              </label>
            </div>
          ...
          <p className="mt-6 text-center text-sm text-amber-700">
            ...
            <Link href="/login" className="font-bold text-amber-900 hover:underline">
              Sign in
            </Link>
          </p>
        </div>
