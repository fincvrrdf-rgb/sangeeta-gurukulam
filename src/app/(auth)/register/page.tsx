...
export default function RegisterPage() {
  ...
  const assignStudentRole = async (uid: string, userEmail: string, name: string, idToken: string) => {
    ...
    if (!res.ok) throw new Error((await res.json()).error || 'Failed to assign role');
  };
      ...
      <div className="w-full lg:w-7/12 flex items-center justify-center px-8 py-16"
        ...
        <div className="w-full max-w-sm">
          ...
          <div className="mb-8">
            ...
            <p className="mt-1 text-sm" style={{ color: '#92400e' }}>Begin your musical journey</p>
          </div>
          ...
          <form onSubmit={handleEmailRegister} className="space-y-4">
            {[
              { id: 'name', label: 'Full Name', type: 'text', value: displayName, setter: setDisplayName, placeholder: 'Your full name' },
              { id: 'email', label: 'Email', type: 'email', value: email, setter: setEmail, placeholder: 'your@email.com' },
              { id: 'password', label: 'Password', type: 'password', value: password, setter: setPassword, placeholder: '6+ characters' },
              ...
              <div key={id}>
                <label htmlFor={id} className="block text-xs font-semibold tracking-widest uppercase mb-2"
                  style={{ color: '#7c2d12' }}>{label}</label>
                <input id={id} type={type} required minLength={type === 'password' ? 6 : undefined}
            ...
            <label className="flex items-start gap-3 cursor-pointer pt-1">
              <input type="checkbox" checked={acceptTerms} onChange={e => setAcceptTerms(e.target.checked)}
                className="mt-0.5 w-4 h-4 rounded" style={{ accentColor: '#b45309' }} />
              <span className="text-xs leading-relaxed" style={{ color: '#7c2d12' }}>
                I accept the{' '}
                <Link href="/terms" className="underline hover:opacity-70">Terms of Service</Link>
                {' '}and{' '}
            ...
            <button type="submit" disabled={loading}
              ...
              style={{ background: loading ? '#d1d5db' : 'linear-gradient(135deg, #92400e, #b45309)', color: '#fff', letterSpacing: '0.1em' }}>
          ...
          <p className="mt-8 text-center text-xs" style={{ color: '#92400e' }}>
            ...
            <Link href="/login" className="font-semibold underline" style={{ color: '#7c2d12' }}>Sign in</Link>
          </p>
        </div>
      </div>
    </div>
